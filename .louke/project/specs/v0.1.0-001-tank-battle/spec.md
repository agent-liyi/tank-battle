...
游戏应在一个固定尺寸的 HTML5 Canvas 元素上渲染。游戏世界是一个 32 列 x 32 行的基于网格的网格，每个格子为 32x32 像素（总游玩区域：1024x1024 像素）。游戏应使用 `requestAnimationFrame` 以 60 帧每秒的速度运行主循环。每一帧应清空画布、更新游戏状态（移动、碰撞、AI），并渲染所有游戏对象（网格、坦克、子弹、基地、UI 覆盖层）。画布应具有深色背景色。
